...
fzf_select_backup_items() {
    ...
    local keys=("ssh" "gnupg" "dconf" "keyrings")
    local selected=()
...
fzf_select_restore_items() {
    ...
    for item in ssh gnupg dconf keyrings; do
        if [ -d "$1/$item" ]; then
            case "$item" in
                ssh)      desc="SSH 配置 (含密钥)" ;;
...
do_backup() {
    ...
    if command -v fzf &>/dev/null; then
        ...
        BACKUP_ITEM_KEYS=("ssh" "gnupg" "dconf" "keyrings")
...
EOF
    ...
    if tar -cpzf "$ARCHIVE_NAME" "backup_${TIMESTAMP}"; then
        ...
        echo ""
        echo -e "${YELLOW}⚠️ 配置文件已备份，请妥善保管。${NC}"
        press_enter
    else
